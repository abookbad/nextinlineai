...
export default function CalendarEmbed({ src }: { src?: string }) {
  ...
  return (
    <Section>
      <Container>
        <h2 className="text-2xl font-semibold text-white">Ready to see it for your team?</h2>
        <div className="mt-4 overflow-hidden rounded-2xl border border-white/20 bg-white/5">
          {calendarSrc ? (
            <iframe title="Booking calendar" src={calendarSrc} className="h-[720px] w-full" loading="lazy" />
          ) : (
            <div className="p-6 text-white/80">Calendar embed coming soon.</div>
          )}
        </div>
        <div className="mt-4">
          <TrackedLink href="/book" prefetch data-analytics-id="cta-calendar-cleaning" analytics={{ eventName: "cta_click", payload: { location: "calendar" } }}>
